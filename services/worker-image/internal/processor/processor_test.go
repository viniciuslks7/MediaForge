...
func TestProcessProducesRequestedVariants(t *testing.T) {
	...
	results, err := p.Process(src, []string{"resize", "thumbnail", "webp"})
	if err != nil {
...
func TestProcessUnknownOperationsIgnored(t *testing.T) {
	src := makePNG(t, 64, 64)
	p := New(Options{})
	results, err := p.Process(src, []string{"resize", "bogus"})
	if err != nil {
		t.Fatalf("process: %v", err)
	}
	if len(results) != 1 {
...
func TestProcessRejectsNonImage(t *testing.T) {
	p := New(Options{})
	if _, err := p.Process([]byte("not an image"), []string{"resize"}); err == nil {
